...
export default function CreateClientPage() {
  ...
  const handleBasicInfoSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        console.log('Client created successfully, selectedTeamMembers:', selectedTeamMembers);
        ...
        if (selectedTeamMembers.length > 0) {
          ...
          for (const teamMemberId of selectedTeamMembers) {
            ...
            const actualTeamMemberId = teamMemberId === 'me' ? -1 : Number(teamMemberId);
            console.log('=== ASSIGNMENT DEBUG ===');
            console.log('Raw teamMemberId from selectedTeamMembers:', teamMemberId);
            console.log('Is "me"?:', teamMemberId === 'me');
            console.log('User ID (trainer ID):', user.id);
            console.log('Final actualTeamMemberId:', actualTeamMemberId);
            console.log('Creating assignment for client:', data.client.id);
            ...
            if (assignmentResponse.ok) {
              ...
              console.error('Error details:', {
                ...
                clientId: data.client.id,
                assignedBy: user.id,
                responseStatus: assignmentResponse.status
              });
            }
          }
          console.log('All team assignments created successfully');
        } else {
          console.log('No team members selected for assignment');
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Error creating client or assignments:', err);
    ...
    <div>
      <div className="max-w-4xl">
        ...
        <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow p-6">
              <h2 className="text-lg font-semibold mb-4">Assign to Team Members</h2>
              <div className="space-y-4">
                <p className="text-sm text-gray-600 mb-2">
                  Select team members who should be assigned to this client
                </p>
                <div className="border border-gray-200 rounded-lg p-3 max-h-60 overflow-y-auto">
                  <div className="space-y-2">
                    <label className="flex items-center space-x-2 cursor-pointer hover:bg-gray-50 rounded p-2">
                      <input
                        type="checkbox"
                        checked={selectedTeamMembers.includes('me')}
                        onChange={(e) => {
                          console.log('Me checkbox clicked, checked:', e.target.checked);
                          if (e.target.checked) {
                            setSelectedTeamMembers([...selectedTeamMembers, 'me']);
                          } else {
                            setSelectedTeamMembers(selectedTeamMembers.filter(id => id !== 'me'));
                          }
                        }}
                        className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                      />
                      <span className="text-sm text-gray-700">Me (Owner)</span>
                    </label>
                    {teamMembers.length > 0 && teamMembers.map((member: any) => (
                      <label key={member.id} className="flex items-center space-x-2 cursor-pointer hover:bg-gray-50 rounded p-2">
                        <input
                          type="checkbox"
                          ...
                          className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                        />
                        <span className="text-sm text-gray-700">{member.fullName || member.email}</span>
                      </label>
                    ))}
                    {teamMembers.length === 0 && (
                      <p className="text-sm text-gray-500 pt-2">No other team members available. Create team members from the Team Members section.</p>
                    )}
                    </div>
                  </div>
                  {selectedTeamMembers.length > 0 && (
                    <div className="mt-2 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                      <p className="text-sm text-blue-800">
                        {selectedTeamMembers.length} team member{selectedTeamMembers.length > 1 ? 's' : ''} will be assigned to this client
                      </p>
                    </div>
                  )}
                </div>
